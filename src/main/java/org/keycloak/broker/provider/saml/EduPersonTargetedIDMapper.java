...
public class EduPersonTargetedIDMapper extends AbstractIdentityProviderMapper implements SamlMetadataDescriptorUpdater {
    ...
    private static final Set<IdentityProviderSyncMode> IDENTITY_PROVIDER_SYNC_MODES = Stream.of(IdentityProviderSyncMode.values()).collect(Collectors.toSet());

    public static final List<String> NAME_FORMATS = Arrays.asList(ATTRIBUTE_FORMAT_BASIC.name(), JBossSAMLURIConstants.ATTRIBUTE_FORMAT_URI.name(), JBossSAMLURIConstants.ATTRIBUTE_FORMAT_UNSPECIFIED.name());
    static {
